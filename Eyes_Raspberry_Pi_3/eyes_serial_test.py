...
#		print(frames/(now-beginningTime))
    ...
    if abs(posP - prevPupilScale) >= irisRegenThreshold:
        # Interpolate points between min and max pupil sizes
        interPupil = points_interp(pupilMinPts, pupilMaxPts, posP)
  ...
  range):     # +/- random pupil scale at midpoint
    ...
    if range >= 0.125: # Limit subdvision count, because recursion
        ...
        while True:
            ...
            frame(pupil_expected_size) # Draw frame w/interim pupil scale value
...
while (end - start < total_time):     
    ...
    if (OP_MODE == 0):
        #print(lux)
        if (lux > 500):
            pupil_expected_size = 0.2
        elif (lux <= 500):
            pupil_expected_size = 0.8

    elif (OP_MODE == 1):
        pupil_size(temp_size / 100)
        #print(pupil_expected_size)
    
    if PUPIL_IN >= 0: # Pupil scale from sensor
        if   pupil_expected_size < PUPIL_MIN: pupil_expected_size = PUPIL_MIN
        elif pupil_expected_size > PUPIL_MAX: pupil_expected_size = PUPIL_MAX
        # Scale to 0.0 to 1.0:
        pupil_expected_size = (pupil_expected_size - PUPIL_MIN) / (PUPIL_MAX - PUPIL_MIN)
        if PUPIL_SMOOTH > 0:
            pupil_expected_size = ((currentPupilScale * (PUPIL_SMOOTH - 1) + pupil_expected_size) / PUPIL_SMOOTH)
        frame(pupil_expected_size)
    else: # Fractal auto pupil scale
        pupil_expected_size = random.random()
        split(currentPupilScale, pupil_expected_size, 4.0, 1.0)
    currentPupilScale = pupil_expected_size
    end = time.time() 
    #print(end - start) 
    #print("\n")
os._exit(1)
